...
<body>
    ...
    <main>
        <!--<div class="horizontal-panel">
            <button id="category" onclick="display('Tops')">Tops</button>
            <button id="category" onclick="display('Bottoms')">Bottoms</button>
            <button id="category" onclick="display('Suits_and_Blazers')">Suits and Blazers</button>
            <button id="category" onclick="display('Outer Wear')">Outer Wear</button>
            <button id="category" onclick="display('Active Wear')">Active Wear</button>
            <button id="category" onclick="display('Swimwear')">Swimwear</button>
            <button id="category" onclick="display('Accessories')">Accessories</button>
            <button id="category" onclick="display('Ethnic Wear')">Ethnic Wear</button>
            <button id="category" onclick="display('Ethinic Bottoms')">Ethinic Bottoms</button>
            <button id="category" onclick="display('Indo-Western Outfits')">Indo-Western Outfits</button>
            <button id="category" onclick="display('Ethinic Accessories')">Ethinic Accessories</button>
            <button id="category" onclick="display('Formal Wear')">Formal Wear</button>
            <button id="category" onclick="display('Wedding Wear')">Wedding Wear</button>
            <button id="category" onclick="display('Party Wear')">Party Wear</button>
        </div>-->

        <div class="horizontal-panel">
      <button class="category-btn" data-category="Tops">Tops</button>
      <button class="category-btn" data-category="Bottoms">Bottoms</button>
      <button class="category-btn" data-category="Suits_and_Blazers">Suits and Blazers</button>
      <button class="category-btn" data-category="Outer_Wear">Outer Wear</button>
      <button class="category-btn" data-category="Active_Wear">Active Wear</button>
      <button class="category-btn" data-category="Swimwear">Swimwear</button>
      <button class="category-btn" data-category="Accessories">Accessories</button>
      <button class="category-btn" data-category="Ethnic_Wear">Ethnic Wear</button>
      <button class="category-btn" data-category="Ethinic_Bottoms">Ethinic Bottoms</button>
      <button class="category-btn" data-category="Indo_Western_Outfits">Indo-Western Outfits</button>
      <button class="category-btn" data-category="Ethinic_Accessories">Ethinic Accessories</button>
      <button class="category-btn" data-category="Formal_Wear">Formal Wear</button>
      <button class="category-btn" data-category="Wedding_Wear">Wedding Wear</button>
      <button class="category-btn" data-category="Party_Wear">Party Wear</button>
    </div>
    ...
    <div class="product-container" id="container">
      <?php
        ...
        if (isset($_GET['category'])) {
          ...
          $sql = "SELECT * FROM products_data WHERE Category_Name = '$category' AND Gender = 'Men'";
        } else {
          $sql = "SELECT * FROM products_data WHERE Product_Type = 'Mens_Clothing' AND Gender = 'Men'";
        }
        ...
        if ($result->num_rows > 0) {
          while ($row = $result->fetch_assoc()) {
            ...
            echo '<img src="data:image/jpeg;base64,' . base64_encode($row['Product_Image']) . '" alt="' . $row['Product_Name'] . '" style="max-width: 100%; height: auto;">';
            echo '<h3>' . $row['Product_Name'] . '</h3>';
            echo '<p>Price: ' . $row['Price'] . '</p>';
            echo '<button type="button">View Product</button>';
            echo '</div>';
          }
        } else {
    ...
    <footer></footer>
    <!-- JavaScript to handle showing product info in the dynamic panel -->
    <script>
...
function display(category){
    window.location.href = 'mens_wear.php?category=' + category;
}
